...
struct TableCellView: View {
    ...
    var backgroundColor: Color {
        ...
        if (table.hasHeaderRow ?? false) && row == 0 {
            return Color.gray.opacity(0.35)
        }
        if (table.hasHeaderColumn ?? false) && column == 0 {
            return Color.gray.opacity(0.2)
        }
        if (table.showAlternatingRowColors ?? false) && row % 2 == 1 {
    ...
    var body: some View {
        ...
        .overlay(
            Rectangle()
                .stroke((table.showBorders ?? false) ? Color.gray.opacity(0.3) : Color.clear, lineWidth: 0.5)
        )
        .overlay(
