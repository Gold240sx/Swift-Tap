...
struct FilePathBlockView: View {
    ...
    private var fallbackIcon: some View {
        Image(systemName: filePathData.isDirectory ? "folder.fill" : "doc.fill")
            .font(.system(size: 36))
            .foregroundStyle(filePathData.isDirectory ? .blue : .secondary)
            .frame(width: 48, height: 48)
    }
    ...
    private var extensionBadge: some View {
        if !filePathData.fileExtension.isEmpty && !filePathData.isDirectory {
    ...
    private func copyPath() {
        ...
        pasteboard.setString(filePathData.pathString, forType: .string)
    }
