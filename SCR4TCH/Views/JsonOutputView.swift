        ...
        for block in (note.blocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) }) {
            ...
            if block.type == .text, let text = block.text {
                ...
                var tDict: [String: Any] = [
                    ...
                    "cols": table.columnCount ?? 0,
                    "columnWidths": table.columnWidths ?? [],
                    "rowHeights": table.rowHeights ?? []
                ]
                var cells: [[String: String]] = []
                for r in 0..<(table.rowCount ?? 0) {
                    for c in 0..<(table.columnCount ?? 0) {
    ...
    private func blockToMarkdown(_ block: NoteBlock) -> String {
        ...
        case .table:
            if let table = block.table {
                ...
                // Separator
                var separator = "|"
                for _ in 0..<(table.columnCount ?? 0) {
                    separator += " --- |"
                }
                md += separator + "\n"
                
                // Data rows
                for r in 1..<(table.rowCount ?? 0) {
                    var row = "|"
                    for c in 0..<(table.columnCount ?? 0) {
        ...
        case .list:
            if let listData = block.listData {
                ...
                for item in (listData.items ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) }) {
                    let prefix: String
                    let type = listData.listType // Not optional
                    switch type {
                    case .bullet:
                        prefix = "- "
                    case .numbered:
                        prefix = "\((item.orderIndex ?? 0) + 1). "
                    case .checkbox:
                        prefix = (item.isChecked ?? false) ? "- [x] " : "- [ ] "
                    }
                    md += "\(prefix)\(item.text ?? "")\n"
                }
                return md
        ...
        case .columns:
            if let columnData = block.columnData {
                ...
                for column in (columnData.columns ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) }) {
                    md += "**Column \((column.orderIndex ?? 0) + 1):**\n\n"
                    for colBlock in (column.blocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) }) {
        ...
        case .reminder:
            if let reminder = block.reminderData {
                 var md = "- [\(reminder.isCompleted ?? false ? "x" : " ")] \(reminder.title ?? "Reminder")"
                 // Add date info if available
                 if (reminder.dueDate ?? Date()) > Date() {
                      md += " (Due: \((reminder.dueDate ?? Date()).formatted()))"
                 }
                 return md
            }
            return ""
