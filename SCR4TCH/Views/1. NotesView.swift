        ...
        if let tagName {
            ...
            if isAllStatus {
               switch categoryName {
                ...
                case Category.all:
                    let predicate = #Predicate<RichTextNote> { note in
                        note.statusRaw == statusRaw && (note.tags?.contains { $0.name == tagName } == true)
                    }
                    _allNotes = Query(filter: predicate)
                case Category.uncategorized:
                    let predicate = #Predicate<RichTextNote> { note in
                        note.statusRaw == statusRaw && note.category == nil && (note.tags?.contains { $0.name == tagName } == true)
                    }
                    _allNotes = Query(filter: predicate)
                default:
                    let predicate = #Predicate<RichTextNote> { note in
                        note.statusRaw == statusRaw && note.category?.name == categoryName && (note.tags?.contains { $0.name == tagName } == true)
                    }
                    _allNotes = Query(filter: predicate)
                }
            }
                    ...
                    HStack(spacing: 8) {
                        ...
                        Menu {
                            ...
                            ForEach(categories) { category in
                                Button {
                                    filterCategory = category.name ?? "Uncategorized"
                                } label: {
                                    Label(category.name ?? "Uncategorized", systemImage: filterCategory == category.name ? "checkmark" : "")
                                }
                            }
                        } label: {
                            HStack(spacing: 4) {
                        ...
                        Menu {
                            ...
                            ForEach(allTags) { tag in
                                Button {
                                    ...
                                    Label(tag.name ?? "Tag", systemImage: filterTag == tag.name ? "checkmark" : "")
                                }
                            }
                        } label: {
                            HStack(spacing: 4) {
