...
struct ImageBlockView: View {
    ...
    private var displayOffsetX: Double {
        dragOffsetX ?? imageData.offsetX
    }

    private var displayOffsetY: Double {
        dragOffsetY ?? imageData.offsetY
    }

    private var displayScale: Double {
        dragScale ?? imageData.scale
    }
    ...
    var body: some View {
        VStack(alignment: .center, spacing: 8) {
            ...
            .frame(maxWidth: imageData.isFullWidth ? .infinity : CGFloat(displayWidth))
            .onHover { hovering in
    ...
    private var imageContainer: some View {
        ZStack {
            ...
            WebImage(url: URL(string: imageData.urlString))
                ...
                .frame(width: imageData.isFullWidth ? nil : CGFloat(displayWidth),
    ...
    private var panGesture: some Gesture {
        DragGesture(minimumDistance: 1)
            .onChanged { value in
                ...
                dragOffsetX = imageData.offsetX + value.translation.width
                dragOffsetY = imageData.offsetY + value.translation.height
            }
            .onEnded { _ in
                commitPan()
            }
    ...
    private func commitPan() {
        imageData.offsetX = dragOffsetX ?? imageData.offsetX
        imageData.offsetY = dragOffsetY ?? imageData.offsetY
        dragOffsetX = nil
        dragOffsetY = nil
        isPanning = false
        #if os(macOS)
        ...
        HStack(spacing: 6) {
            ...
            Button {
                withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) {
                    imageData.isFullWidth.toggle()
                    if !imageData.isFullWidth && imageData.width == nil {
                        imageData.width = 300
                    }
                    try? context.save()
                }
            } label: {
                Image(systemName: imageData.isFullWidth ? "arrow.down.right.and.arrow.up.left" : "arrow.up.left.and.arrow.down.right")
            ...
            .buttonStyle(.plain)
            .help(imageData.isFullWidth ? "Shrink to fixed width" : "Expand to full width")
