...
struct AccordionBlockView: View {
        ...
        HStack(alignment: .top, spacing: 8) {
            ...
            Button {
                withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) {
                    accordion.isExpanded = !(accordion.isExpanded ?? false)
                }
            } label: {
                Image(systemName: "chevron.right")
                    .font(.system(size: 12, weight: .semibold))
                    .foregroundStyle(.secondary)
                    .rotationEffect(.degrees((accordion.isExpanded ?? false) ? 90 : 0))
    ...
    private var expandedContentView: some View {
        if accordion.isExpanded ?? false {
            VStack(alignment: .leading, spacing: 8) {
                ForEach((accordion.contentBlocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })) { block in
                    nestedBlockView(for: block)
                        .padding(4)
                        .background(
                             GeometryReader { geo in
                                 Color.clear.onAppear {
                                    if let id = block.id { blockHeights[id] = geo.size.height }
                                 }
                                .onChange(of: geo.size.height) { _, newHeight in
                                    if let id = block.id { blockHeights[id] = newHeight }
                                }
                             }
                        )
...
struct NestedBlockDropDelegate: DropDelegate {
    ...
    func dropUpdated(info: DropInfo) -> DropProposal? {
        ...
        let height = (block.id != nil ? blockHeights[block.id!] : 0) ?? 0
