...
struct ListBlockView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            // List items
            ForEach((listData.items ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })) { item in
                if let id = item.id {
                    ListItemView(
                        item: item,
    ...
    private func getItemNumber(for item: ListItem) -> Int {
        let sorted = (listData.items ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })
    ...
    private func insertItemAfter(_ item: ListItem) {
        let sorted = (listData.items ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })
        let targetIndex = (item.orderIndex ?? 0) + 1
        ...
        for i in (listData.items ?? []) where (i.orderIndex ?? 0) >= targetIndex {
            i.orderIndex = (i.orderIndex ?? 0) + 1
        }

        // Create new item
        let newItem = ListItem(orderIndex: targetIndex, text: suffixText)
        newItem.parentList = listData
        if listData.items == nil { listData.items = [] }
    ...
    private func removeItem(_ item: ListItem) {
        let removedIndex = item.orderIndex ?? 0
        if var items = listData.items {
        ...
        // Re-index
        for i in (listData.items ?? []) where (i.orderIndex ?? 0) > removedIndex {
            i.orderIndex = (i.orderIndex ?? 0) - 1
        }
