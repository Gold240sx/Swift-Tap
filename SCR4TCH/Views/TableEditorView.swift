...
struct TableEditorView: View {
                    ...
                    ScrollView(.horizontal, showsIndicators: true) {
                        VStack(spacing: 0) {
                            ...
                            if isEditing {
                                HStack(spacing: 0) {
                                    cornerCell
                                    ForEach(0..<(table.columnCount ?? 0), id: \.self) { col in
                            ...
                            VStack(spacing: 0) {
                                ForEach(0..<(table.rowCount ?? 0), id: \.self) { row in
                                    HStack(spacing: 0) {
                                        if isEditing {
                                            rowNumberCell(row)
                                        }
    ...
    private var headerSection: some View {
        if !(table.showTitle ?? false) && !isHovering && !isEditing && !showSettings && !showJSONPopover {
            EmptyView()
        } else {
            HStack {
                if isEditing {
                ...
                if table.showTitle ?? false {
                    TextField("Table Name", text: Binding(get: { table.title ?? "" }, set: { table.title = $0 }))
                        .font(.headline)
    ...
    private var addColumnButton: some View {
        if isEditing {
            Button {
                ...
                var targetIndex = table.columnCount ?? 0
    ...
    private func copySelectedContent() {
        ...
        if !selectedRows.isEmpty {
            cellsToCopy = selectedRows.flatMap { r in (0..<(table.columnCount ?? 0)).map { c in CellIndex(row: r, col: c) } }
        } else if !selectedColumns.isEmpty {
            cellsToCopy = selectedColumns.flatMap { c in (0..<(table.rowCount ?? 0)).map { r in CellIndex(row: r, col: c) } }
    ...
    private func pasteFromClipboard() {
        ...
        for (rOffset, line) in lines.enumerated() {
            ...
            for (cOffset, cellText) in cells.enumerated() {
                ...
                if r < (table.rowCount ?? 0) && c < (table.columnCount ?? 0) {
    ...
    private func calculateTableWidth() -> CGFloat {
        ...
        for col in 0..<(table.columnCount ?? 0) {
...
struct TableSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Toggle("Header Row", isOn: Binding(get: { table.hasHeaderRow ?? false }, set: { table.hasHeaderRow = $0 }))
            Toggle("Header Column", isOn: Binding(get: { table.hasHeaderColumn ?? false }, set: { table.hasHeaderColumn = $0 }))
            Toggle("Show Title", isOn: Binding(get: { table.showTitle ?? false }, set: { table.showTitle = $0 }))
            Toggle("Alternating Row Colors", isOn: Binding(get: { table.showAlternatingRowColors ?? false }, set: { table.showAlternatingRowColors = $0 }))
            Toggle("Show Borders", isOn: Binding(get: { table.showBorders ?? false }, set: { table.showBorders = $0 }))
