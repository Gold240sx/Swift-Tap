...
struct NotesEditorView: View {
        ...
        ScrollViewReader { proxy in
            GeometryReader { scrollGeo in
                ScrollView {
                    ...
                    ForEach((note.blocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })) { block in
                    blockRowView(for: block)
                        ...
                        .background(
                             GeometryReader { geo in
                                 ...
                                 return Color.clear
                                     .preference(key: BlockPositionPreferenceKey.self, value: [block.id ?? UUID(): relativeY])
                                     .onAppear {
                                         if let id = block.id {
                                             blockHeights[id] = geo.size.height
                                             blockPositions[id] = relativeY
                                         }
                                     }
                        ...
                        .onDrop(of: [UTType.noteBlock], delegate: BlockDropDelegate(
                            ...
                            blockPosition: blockPositions[block.id ?? UUID()] ?? 0,
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            if focusedBlockID == nil {
                if let firstTextBlock = (note.blocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) }).first(where: { $0.type == .text }) {
        ...
        BlockRowHoverContainer(block: block) { isHovered in
            HStack(alignment: .top, spacing: 6) {
                ...
                BlockControlsView(
                    ...
                    onInsertReminderAfter: { insertReminderAfterBlock($0) },
    ...
    private func insertBlockAtRoot(_ createBlock: (Int) -> NoteBlock) {
        let sortedBlocks = (note.blocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })
        ...
        if let focusedID = effectiveFocusedID,
           let focusedBlock = sortedBlocks.first(where: { $0.id == focusedID }) {
            ...
            if isTextBlockEmpty(focusedBlock) {
                let orderIndex = focusedBlock.orderIndex ?? 0
            ...
            targetOrderIndex = (focusedBlock.orderIndex ?? 0) + 1
            ...
            if let selection = selections[focusedID],
               ...
               !text.characters.isEmpty {
                ...
                if let index = splitIndex, index < text.endIndex {
                     ...
                     for block in note.blocks ?? [] where (block.orderIndex ?? 0) >= targetOrderIndex {
                         block.orderIndex = (block.orderIndex ?? 0) + 2
                     }
        ...
        for block in note.blocks ?? [] where (block.orderIndex ?? 0) >= targetOrderIndex {
            block.orderIndex = (block.orderIndex ?? 0) + 1
        }
    ...
    private func extractSelection(from block: NoteBlock) {
        guard let text = block.text, !text.characters.isEmpty,
              let id = block.id,
              let selection = selections[id] else { return }
        ...
        // Base order index
        let baseIndex = block.orderIndex ?? 0
        ...
        if neededSlots > 0 {
            for b in note.blocks ?? [] where (b.orderIndex ?? 0) > baseIndex {
                b.orderIndex = (b.orderIndex ?? 0) + neededSlots
            }
        }
    ...
    private func insertTableInAccordion(_ accordion: AccordionData, rows: Int, cols: Int) {
        ...
        let sortedBlocks = (accordion.contentBlocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })
        let targetOrderIndex = sortedBlocks.count

        // Shift subsequent blocks
        for block in accordion.contentBlocks ?? [] where (block.orderIndex ?? 0) >= targetOrderIndex {
            block.orderIndex = (block.orderIndex ?? 0) + 1
        }

        let tableBlock = NoteBlock(orderIndex: targetOrderIndex, table: newTable, type: .table)
        tableBlock.parentAccordion = accordion
        if accordion.contentBlocks == nil { accordion.contentBlocks = [] }
    ...
    private func insertAccordionInAccordion(_ parentAccordion: AccordionData, level: AccordionData.HeadingLevel) {
        ...
        let sortedBlocks = (parentAccordion.contentBlocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })
        let targetOrderIndex = sortedBlocks.count

        // Shift subsequent blocks
        for block in parentAccordion.contentBlocks ?? [] where (block.orderIndex ?? 0) >= targetOrderIndex {
            block.orderIndex = (block.orderIndex ?? 0) + 1
        }
        ...
        if parentAccordion.contentBlocks == nil { parentAccordion.contentBlocks = [] }
    ...
    private func insertColumns(ratios: [Double]) {
        ...
        print("Inserted ColumnData with \(newColumnData.columns?.count ?? 0) columns.")
        
        insertBlockAtRoot { index in
            let block = NoteBlock(orderIndex: index, columnData: newColumnData, type: .columns)
            // Focus first block of first column
            if let firstCol = (newColumnData.columns ?? []).sorted(by: {($0.orderIndex ?? 0) < ($1.orderIndex ?? 0)}).first,
                ...
                DispatchQueue.main.async {
                    ...
                    if let focusedBlock = focusedBlock,
                       self.isTextBlockEmpty(focusedBlock) {
                        ...
                        let targetIndex = (focusedBlock.orderIndex ?? 0) + 1

                        for block in column.blocks ?? [] where (block.orderIndex ?? 0) >= targetIndex {
                            block.orderIndex = (block.orderIndex ?? 0) + 1
                        }
                        ...
                        // Append at end
                        let sortedBlocks = (column.blocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })
    ...
    private func removeBlockFromColumn(_ block: NoteBlock) {
        ...
        if let parentColumn = block.parentColumn {
            ...
            let sorted = (parentColumn.blocks ?? []).sorted(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })
    ...
    private func removeBlockFromAccordion(_ block: NoteBlock) {
        ...
        let removedOrder = block.orderIndex ?? 0
        context.delete(block)
        parentAccordion.contentBlocks?.removeAll(where: { $0.id == block.id })

        // Re-index remaining blocks
        for b in parentAccordion.contentBlocks ?? [] where (b.orderIndex ?? 0) > removedOrder {
    ...
    private func removeBlock(_ block: NoteBlock) {
        ...
        let prevBlock = (note.blocks ?? []).first(where: { (b: NoteBlock) -> Bool in (b.orderIndex ?? 0) == (block.orderIndex ?? 0) - 1 })
        let nextBlock = (note.blocks ?? []).first(where: { (b: NoteBlock) -> Bool in (b.orderIndex ?? 0) == (block.orderIndex ?? 0) + 1 })
        ...
        // Re‑index remaining blocks
        for b in note.blocks ?? [] where (b.orderIndex ?? 0) > removedOrder {
            b.orderIndex = (b.orderIndex ?? 0) - 1
        }
        ...
        if block.type != .text {
            if let prev = prevBlock, let next = nextBlock,
               ...
               let prevText = prev.text, let nextText = next.text {
                ...
                for b in note.blocks ?? [] where (b.orderIndex ?? 0) > (next.orderIndex ?? 0) {
                    b.orderIndex = (b.orderIndex ?? 0) - 1
                }
            }
        }
    ...
    private func duplicateBlock(_ block: NoteBlock) {
        ...
        let insertIndex = (block.orderIndex ?? 0) + 1

        // Shift subsequent blocks
        for b in note.blocks ?? [] where (b.orderIndex ?? 0) >= insertIndex {
            b.orderIndex = (b.orderIndex ?? 0) + 1
        }
    ...
    private func pasteBlockAfter(_ targetBlock: NoteBlock) {
        ...
        let insertIndex = (targetBlock.orderIndex ?? 0) + 1
    ...
    private func pasteBlockAfterInAccordion(_ targetBlock: NoteBlock, accordion: AccordionData) {
        ...
        // Shift subsequent blocks
        for b in accordion.contentBlocks ?? [] where (b.orderIndex ?? 0) >= insertIndex {
            b.orderIndex = (b.orderIndex ?? 0) + 1
        }
    ...
    private func pasteBlockAfterInColumn(_ targetBlock: NoteBlock, column: Column) {
        ...
        // Shift subsequent blocks
        for b in column.blocks ?? [] where (b.orderIndex ?? 0) >= insertIndex {
            b.orderIndex = (b.orderIndex ?? 0) + 1
        }
    ...
    private func handleBlockDrop(dragged: NoteBlock, target: NoteBlock, edge: DropEdge) {
        ...
        if let targetParent = target.parentAccordion {
            ...
            var blocks = targetParent.contentBlocks ?? []
            insertBlock(dragged, into: &blocks, relativeTo: target, edge: edge)
            targetParent.contentBlocks = blocks
            ...
            var blocks = targetColumn.blocks ?? []
            insertBlock(dragged, into: &blocks, relativeTo: target, edge: edge)
            targetColumn.blocks = blocks
    ...
    private func insertBlock(_ block: NoteBlock, into list: inout [NoteBlock], relativeTo target: NoteBlock, edge: DropEdge) {
        ...
        list.sort(by: { ($0.orderIndex ?? 0) < ($1.orderIndex ?? 0) })
    ...
    private func insertFilePathAtFocusedBlock(url: URL, focusedID: UUID?) {
        ...
        if isTextBlockEmpty(focusedBlock) {
            ...
            // Shift subsequent blocks
            for block in note.blocks ?? [] where (block.orderIndex ?? 0) >= targetIndex {
                block.orderIndex = (block.orderIndex ?? 0) + 1
            }
    ...
    private func insertTextBlockAfter(_ block: NoteBlock) {
        let targetIndex = (block.orderIndex ?? 0) + 1

        // Shift subsequent blocks
        for b in note.blocks ?? [] where (b.orderIndex ?? 0) >= targetIndex {
            b.orderIndex = (b.orderIndex ?? 0) + 1
        }
        ...
        panel.begin { response in
            if response == .OK, let url = panel.url {
                DispatchQueue.main.async {
                    ...
                    let targetIndex = (targetBlock.orderIndex ?? 0) + 1
                    
                    // Shift subsequent blocks
                    for b in (self.note.blocks ?? []) where (b.orderIndex ?? 0) >= targetIndex {
                        b.orderIndex = (b.orderIndex ?? 0) + 1
                    }
                    ...
                    let newBlock = NoteBlock(orderIndex: targetIndex, filePathData: filePathData, type: .filePath)
                    if self.note.blocks == nil { self.note.blocks = [] }
    ...
    private func insertReminderAfterBlock(_ block: NoteBlock) {
        let targetIndex = (block.orderIndex ?? 0) + 1
        ...
        let reminderData = ReminderData(dueDate: Date().addingTimeInterval(3600))
        context.insert(reminderData)
        
        let newBlock = NoteBlock(orderIndex: targetIndex, reminderData: reminderData, type: .reminder)
    ...
    private func insertTextBlockAfterInAccordion(_ block: NoteBlock, accordion: AccordionData) {
        ...
        // Shift subsequent blocks
        for b in (accordion.contentBlocks ?? []) where (b.orderIndex ?? 0) >= targetIndex {
            b.orderIndex = (b.orderIndex ?? 0) + 1
        }
    ...
    private func insertTextBlockAfterInColumn(_ block: NoteBlock, column: Column) {
        ...
        // Shift subsequent blocks
        for b in (column.blocks ?? []) where (b.orderIndex ?? 0) >= targetIndex {
            b.orderIndex = (b.orderIndex ?? 0) + 1
        }
        ...
        Task {
            ...
            await MainActor.run {
                ...
                if let afterBlock = afterBlock {
                    ...
                    let targetIndex = (afterBlock.orderIndex ?? 0) + 1
...
struct BlockDropDelegate: DropDelegate {
    ...
    func dropEntered(info: DropInfo) {
        guard let dragging = draggingBlock, dragging.id != block.id, let id = block.id else { return }
        // Initial state, will be updated in dropUpdated
        dropState = DropState(targetID: id, edge: .bottom)
    }
    ...
    func performDrop(info: DropInfo) -> Bool {
        ...
        if let id = block.id, state.targetID == id {
    ...
    func dropExited(info: DropInfo) {
        if let id = block.id, dropState?.targetID == id {
            dropState = nil
        }
    }
}
...
struct BlockControlsView: View {
    ...
    let onInsertReminderAfter: (NoteBlock) -> Void
