        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack {
                ...
                HStack(spacing: 8) {
                    // Line numbers toggle
                    Button {
                        codeBlock.showLineNumbers.toggle()
                    } label: {
                        Image(systemName: codeBlock.showLineNumbers ? "list.number" : "list.bullet")
                            .font(.system(size: 12))
                            .foregroundStyle(.secondary)
                    }
                    .buttonStyle(.plain)
            ...
            if isEditing {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(alignment: .top, spacing: 0) {
                        ...
                        // Highlighted code
                        Text(highlighter.highlight(codeBlock.code, language: codeBlock.languageString))
                            .font(.system(size: 13, design: .monospaced))
                            .textSelection(.enabled)
                            .padding(12)
                            .frame(maxWidth: .infinity, alignment: .leading)
                            .id(codeBlock.languageString) // Force redraw when language changes
                    }
                }
                .background(theme.background)
            }
